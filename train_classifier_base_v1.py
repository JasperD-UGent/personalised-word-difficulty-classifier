...
def load_lexcomspal2() -> Tuple[Dict, Dict, Dict]:
    """Load LexComSpaL2 corpus from cloned repository in `input_v1` directory (see read me for detailed instructions).
    :return: A dictionary containing the LexComSpaL2 annotations (sentence>target word>participant), a dictionary
        containing the participant features (ID, proficiency level, years of experience, and native language), and a
        dictionary containing the predefined dataset splits of LexComSpaL2 into training, validation, and test sets for
        the purpose of cross-validation.
    """
    path_direc_lexcomspal2 = os.path.join("input_v1", "LexComSpaL2")

    # construct dictionary containing the LexComSpaL2 annotations
    d_dataset_annots = {}

    with open(os.path.join(path_direc_lexcomspal2, "LexComSpaL2_all.tsv"), mode="r", encoding="utf-8") as f_annots:
        ...
        for row in l_rows:
            ...
            if sent_id not in d_dataset_annots:
                d_dataset_annots[sent_id] = {"sent_text": row[2], "annots_per_token": {}}

            d_dataset_annots[sent_id]["annots_per_token"][idx_word] = d_annots_tok

    f_annots.close()

    # construct dictionary containing the participant features
    d_partp_feats = {}

    with open(
            os.path.join(path_direc_lexcomspal2, "LexComSpaL2_participantFeatures.tsv"), mode="r", encoding="utf-8"
    ) as f_partp_feats:
        f_tsv_partp_feats = csv.reader(f_partp_feats, delimiter="\t")
        l_headers = next(f_tsv_partp_feats)
        l_rows = [row for row in f_tsv_partp_feats]

        for row in l_rows:
            partp_id = int(row[0])
            d_partp_feats[partp_id] = {}

            for idx in range(1, 4):
                d_partp_feats[partp_id][l_headers[idx]] = int(row[idx])

    f_partp_feats.close()

    # load dictionary containing the predefined dataset splits
    d_dataset_split = load_json(os.path.join(path_direc_lexcomspal2, "LexComSpaL2_train_val_test_split.json"))

    return d_dataset_annots, d_partp_feats, d_dataset_split
...
def define_features_to_train_classifier(
        ...
        n_partps: int, n_l1s: int, max_sent_length: int, max_word_length: int
) -> Tuple[Dict, Dict]:
        ...
        for partp in tqdm([loop + 1 for loop in range(n_partps)], desc="Define features per participant ..."):
            ...
            for sent in d_dataset_annots:
                ...
                for dic in [
                    ...
                    d_n_years_experience_embeddings, d_l1_embeddings, d_labels
                ]:
...
def train_classifier(
        device: str, d_chars_to_idxs: Dict, d_dataset_annots: Dict, d_dataset_split: Dict,
        n_partps: int, n_l1s: int, max_sent_length: int, max_word_length: int,
    ...
    path_plot = os.path.join(path_direc_visualisation, "classifier_base_architecturePlot.png")
    ...
    with tf.device(device):
        ...
        for fold in range(n_folds_cv):
            ...
            concat_1 = keras.layers.Concatenate(name="concat_1")(
                [flat, inp_word_1, inp_word_2, inp_word_3, inp_word_4, inp_word_5]
            )
            ...
            concat_2 = keras.layers.Concatenate(
                name="concat_2"
            )([bilstm, inp_word_2, inp_word_3, inp_word_4, inp_word_5])
            ...
            classifier = keras.models.Model(
                inputs=[inp_char_1, inp_word_1, inp_word_2, inp_word_3, inp_word_4, inp_word_5],
            ...
            classifier.save(os.path.join(path_direc_weights, f"classifier_base_fold{fold}.keras"))
    ...
    with open(os.path.join(path_direc_performance, "performance_per_fold_base.tsv"), "w", newline="") as f_tsv:
...
def main(path_fasttext_vectors: str, device: str, n_folds_cv: int, n_epochs: int, batch_size: int) -> None:
    ...
    if n_folds_cv > 10:
        print(f"Setting number of cross-validation folds to the maximum number of 10 ({n_folds_cv} folds given).")
        n_folds_cv = 10
    ...
    d_dataset_annots, d_partp_feats, d_dataset_split = load_lexcomspal2()
    ...
    d_feats, d_labels = define_features_to_train_classifier(
        ...
        n_partps, n_l1s, max_sent_length, max_word_length
    )

    # train BiLSTM classifier
    train_classifier(
        device, d_chars_to_idxs, d_dataset_annots, d_dataset_split,
        n_partps, n_l1s, max_sent_length, max_word_length,
